...
class TestUserProfile(BaseTest):
    ...
    def test_get_edit_profile(self):
        ...
        self.assertInHTML(
            ...
            f'class="input input-bordered border-2 text-lg w-full" required '
            f'id="id_email">',
            response.content.decode("utf-8"),
        )
        self.assertInHTML(
            f'<input type="text" name="first_name" value="{self.user.first_name}" placeholder="John"'
            f'class="input input-bordered border-2 text-lg w-full" required id="id_first_name">',
            response.content.decode("utf-8"),
        )
        self.assertInHTML(
            f'<input type="text" name="last_name" value="{self.user.last_name}"'
            f'placeholder="Appleseed"'
            f'class="input input-bordered border-2 text-lg w-full" id="id_last_name"> ',
            response.content.decode("utf-8"),
        )
        self.assertInHTML(
            f'<input type="text" name="phone_number" value="{self.user.phone_number}" placeholder="+13334445555" '
            f'class="input input-bordered border-2 text-lg w-full" id="id_phone_number">',
            response.content.decode("utf-8"),
        )
